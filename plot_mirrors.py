...
def get_mirror_positions_and_ids(mirrors):
    """
    Extract mirror X/Y positions and mirror IDs from a Mirrors object.

    Parameters
    ----------
    mirrors : Mirrors
        Mirrors object containing mirror panel data including positions
        (as read from the mirror list file, in centimeters).

    Returns
    -------
    tuple
        (x_pos, y_pos, diameter, mirror_ids): X and Y positions and mirror
        panel diameter in cm (numpy arrays), and the mirror panel IDs (falls
        back to a 0-based range if the mirror list file has no
        ``mirror_panel_id`` column).
    """
    x_pos = mirrors.mirror_table["mirror_x"].to("cm").value
    y_pos = mirrors.mirror_table["mirror_y"].to("cm").value
    diameter = mirrors.mirror_table["mirror_diameter"].to("cm").value
    mirror_ids = (
        mirrors.mirror_table["mirror_panel_id"]
        if "mirror_panel_id" in mirrors.mirror_table.colnames
        else list(range(len(x_pos)))
    )
    return x_pos, y_pos, diameter, mirror_ids


def plot_mirror_profile(r_pos, z_pos, telescope_model_name=None):
    """
    Plot the mirror panel profile (Z position vs. radial distance).

    Parameters
    ----------
    r_pos : numpy.ndarray
        Radial distance of each mirror panel from the optical axis.
    z_pos : numpy.ndarray
        Z position of each mirror panel.
    ...
    """
    logger.info(f"Plotting mirror profile for {telescope_model_name}")

    fig, ax = plt.subplots(figsize=(8, 6))

    ax.scatter(r_pos, z_pos, s=10, **PATCH_STYLE)
    ax.set_xlabel("Radius [cm]", fontsize=12)
    ax.set_ylabel("z [cm]", fontsize=12)
    title = "Mirror profile"
...
def get_ring_segment_positions_and_ids(data_file_path):
    """
    Extract ring-segment center positions and segment IDs from a ring segmentation file.

    Uses the same ring geometry as plot_mirror_ring_segmentation (ring
    definitions of nseg, rmin, rmax, dphi, phi0) to compute the (x, y) center
    position of each segment, at mid-radius and mid-angle.

    Parameters
    ----------
    data_file_path : Path or str
        Path to the ring segmentation data file, with lines of the form
        "ring <nseg> <rmin> <rmax> <dphi> <phi0>".

    Returns
    -------
    tuple
        (x_pos, y_pos, segment_ids): X and Y center positions (numpy arrays,
        same units as the ring radii in the file) and a running list of
        segment IDs, one per ring segment, ordered ring by ring.
    """
    rings = _read_ring_segmentation_data(data_file_path)

    x_pos, y_pos, segment_ids = [], [], []
    segment_id = 0

    for ring in rings:
        r_mid = (ring["rmin"] + ring["rmax"]) / 2
        for i in range(ring["nseg"]):
            theta_mid = np.deg2rad(i * ring["dphi"] + ring["phi0"] + ring["dphi"] / 2)
            x_pos.append(r_mid * np.cos(theta_mid))
            y_pos.append(r_mid * np.sin(theta_mid))
            segment_id += 1
            segment_ids.append(segment_id)

    return np.array(x_pos), np.array(y_pos), segment_ids
...
def get_shape_segment_positions_and_ids(data_file_path):
    """
    Extract shape-segment positions and segment IDs from a shape segmentation file.

    Uses the same reader as plot_mirror_shape_segmentation
    (_read_shape_segmentation_file) to extract segment center positions and
    IDs, without applying the plot-only clockwise rotation.

    Parameters
    ----------
    data_file_path : Path or str
        Path to the segmentation data file containing explicit shape
        definitions (hex, yhex) with positions, diameters, and rotations.

    Returns
    -------
    tuple
        (x_pos, y_pos, diameter, segment_ids): X, Y positions and segment
        diameters (numpy arrays) and the list of segment IDs, one per shape
        segment.
    """
    shape_segments, segment_ids = _read_shape_segmentation_file(data_file_path)

    x_pos = np.array([seg["x"] for seg in shape_segments])
    y_pos = np.array([seg["y"] for seg in shape_segments])
    diameter = np.array([seg["diameter"] for seg in shape_segments])

    return x_pos, y_pos, diameter, segment_ids
